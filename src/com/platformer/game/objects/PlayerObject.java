...
public class PlayerObject extends GameObject{
	...
	public void tick(LinkedList<GameObject> object) {
		...
		// GRANICE PORUSZANIA SIE PO POZIOMIE
		if (x < 10) x = 10;
		if (x > 232 * 64) x = 232*64;

		// SMIERC PRZY SPADNIECIU W DOL
		if (y > 1130) health = 0;
				...
				if (tempObject.isVisible()) {
					if (getWholeBounds().intersects(tempObject.getBounds())) {
						...
						try {
							...
							oos.close();
							MainClass.logging(false, Level.INFO, "Plik Collectibles " +MainClass.collectiblesFile.getName() +" został poprawnie zapisany.");
						} catch (IOException ioe) {
                            MainClass.logging(false, Level.WARNING, "Błąd zapisu plików Collectibles " +MainClass.collectiblesFile.getName());
